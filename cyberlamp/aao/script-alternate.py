...
subprocess.call(['Rscript', '02a-createdata_alternate.R', filepath, family, str(nobs), str(ngrid), str(knots), str(B), seed] + [str(p) for p in params])
subprocess.call(['Rscript', '03-firstchunk.R', filepath, 'init_args.rds', filepath, str(nbatch)])
