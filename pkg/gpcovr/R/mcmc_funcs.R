...
#'   \code{\link{natspl}} or \code{\link{natspl_ns}}
...
rmatern <- function(n, nu, alpha, sigma, df)
{
  ...
  while(length(samps) < n)
  {
    ...
    ratio <- dmatern(t, nu, alpha, sigma) / (1.01 * scale * dt(t, df = df))
    accept <- runif(5*n) < ratio
...
{
  dmatern(x, nu, alpha, sigma) / dt(x, df)
}
...
{
  xmax <- 1000 * alpha
  optimize(rat_matern_t, interval = c(0, xmax), maximum = T, nu = nu, alpha = alpha, sigma = sigma, df = df)$objective
}
...
#' @param sigma scale parameter
#' @param tau nugget effect
#' @param numSamps the number of MC samples to use. Ignored if \code{mc} is not null.
#' @param mc vector of MC samples. If \code{NULL}, generate them with calls to \code{\link{rmatern}} and \code{\link{covar_mc}}.
...
get_error_bounds <- function(b, x, knots)
{
  ...
  quantiles <- t(apply(curves, 1, function(x) quantile(x, c(0.025, 0.975))))
  quantiles
}
