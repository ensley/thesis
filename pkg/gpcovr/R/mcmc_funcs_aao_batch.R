...
#'
#' This and \code{\link{estbeta_aao}} probably should not be two separate functions
...
#' @param sigma.m The \code{beta} proposal variance hyperparameter
...
#'
#' @return Returns a list of information to pass to \code{\link{estbeta_aao}}. Also writes output to some csv files.
#' @export
#'
estbeta_aao_initialize <- function(filename, errname, nbatch, H, Y, B, init_beta, init_tau, knots, sigma.m, v_tau, t_v, r.opt = 0.23, r = 5, eps = 1e-3, nugget = 1e-3, errthin = 100) {
  ...
  for(i in 2:nbatch) {
    ...
    if(i > 2) {
      ...
      if(i == 2) {
        ...
        r.hat <- mean(diff(beta_samps[1:(i-1),1]) != 0)
        r.hat.tau <- mean(diff(tau_samps[1:(i-1)]) != 0)
      } else {
        r.hat <- mean(diff(beta_samps[(i-r):(i-1),1]) != 0)
        r.hat.tau <- mean(diff(tau_samps[(i-r):(i-1)]) != 0)
      }
      cat('r.hat =', r.hat, '\n')
      v_tau <- exp(log(v_tau) + gamma1*(r.hat.tau - r.opt))
      # sigma.m <- exp(log(sigma.m) + gamma1*(r.hat - r.opt))
      sigma.m <- 1e-4
    }

    # propose beta
    cat('variance constant =', sigma.m, '\n')
    beta_star <- aao_rq_beta(beta, sigma.m)
    ...
    numprior <- aao_dprior_beta(beta_star, tau)
    denomprior <- aao_dprior_beta(beta, tau)
    ...
    # propose tau
    tau_star <- aao_rq_tau(tau, v_tau)
    # calculate acceptance probability
    num <- aao_dprior(beta, tau_star, t_v)
    denom <- aao_dprior(beta, tau, t_v)
    logr <- num - denom
    # accept or reject
...
#'   likelihood calculation. A value of around 100 is recommended.
#'
#' @return Returns a list of information to pass to \code{\link{estbeta_aao}}. Also
#'   writes output to some csv files.
#' @export
#'
estbeta_aao <- function(filename, errname, nbatch, batchidx, H, Y, B, last_few_beta, last_few_tau, prev_lik, last_accept, knots, sigma.m, v_tau, t_v, r.opt = 0.23, eps, nugget = 1e-3, errthin = 100) {
  filecon <- file(filename, 'a+')
  errcon <- file(errname, 'a+')
  # define constants
  M <- length(Y)
  ...
  for(i in 1:nbatch) {
    ...
    r.hat <- mean(diff(beta_samps[i:(i+r-1),1]) != 0)
    cat('r.hat =', r.hat, '\n')
...
#' @export
#'
estbeta_aao_initialize2 <- function(filename, errname, nbatch, H, Y, B, init_beta, init_tau, knots, sigma.m, v_tau, t_v, r.opt = 0.23, r = 5, eps = 1e-3, nugget = 1e-3, errthin = 100) {
  if (file.exists(filename)) file.remove(filename)
  if (file.exists(errname)) file.remove(errname)
  filecon <- file(filename, 'a+')
  errcon <- file(errname, 'a+')
  ...
  tau_samps[1] <- tau <- init_tau
  cat('i = 1\n')
  prev_lik <- lik_star <- likelihood2(beta, knots, Y, H, B, nugget, debug = FALSE)
  last_accept <- TRUE
  ...
  for(i in 2:nbatch) {
    ...
    cat('last draw was', ifelse(last_accept, 'ACCEPTED', 'REJECTED'), '\n')
    cat('last likelihood =', prev_lik, '\n')
    lik_star <- likelihood2(beta_star, knots, Y, H, B, nugget, debug = FALSE)
    cat('proposed likelihood =', lik_star, '\n')
    # calculate acceptance probability
    numprior <- aao_dprior_beta(beta_star, tau)
    denomprior <- aao_dprior_beta(beta, tau)
...
#' @export
#'
estbeta_aao2 <- function(filename, errname, nbatch, batchidx, H, Y, B, last_few_beta, last_few_tau, prev_lik, last_accept, knots, sigma.m, v_tau, t_v, r.opt = 0.23, eps, nugget = 1e-3, errthin = 100) {
  filecon <- file(filename, 'a+')
  errcon <- file(errname, 'a+')
  # define constants
  M <- nrow(Y)
